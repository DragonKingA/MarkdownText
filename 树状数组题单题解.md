...
并且本题输入数据已经给 `y` 和 `x` 都进行升序排序，故直接处理即可。
...
int main()
{
    ...
    for(int i = 0; i < n; i++)
    {
        ...
        scanf("%d%d", &x, &y);
        ++x, ++y;
        level = sum(x);//得到该点划定的矩形范围内点数即该点等级
        cnt[level]++;
        update(x, 1);  //该点横坐标可给后续符合偏序关系的点计数，加入权值数组中
    }
...
故满足二维偏序关系的三元组<i,m,j>实际可以很多。
$$
不难想到先拆分成**二元升序对**，取**中间元素** `Am` 为参照，**分治两种偏序关系**：`Ai < Am (i < m)` 和 `Am < Aj (m < j)`，而每组同时符合两种条件的偏序对都是答案。

先**分治**两种偏序关系分别产生的合法值对，再根据乘法原理得出同时满足两种偏序关系的**两个合法值对的组合情况总数**：
